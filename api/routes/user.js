...
router.get("/", verifyAdminAccess, celebrate({ query: userSchema.query }), async (req, res) => {
...
router.put("/:id", verifyAuthorization, celebrate({ body: userSchema.update }), async (req, res) => {
  ...
  try {
    await User.findByIdAndUpdate(req.params.id, { $set: { fullname, password } }, { new: true });
...
router.get("/stats", verifyAdminAccess, async (req, res) => {
  ...
  try {
    ...
    res.json(data); // 🟢 Phải trả đúng JSON mảng
  } catch (err) {
    console.error(err);
...
// Update quyền admin - chỉ dành cho admin
router.put("/admin/:id", verifyAdminAccess, async (req, res) => {
  const { isAdmin } = req.body;
  try {
    await User.findByIdAndUpdate(req.params.id, { $set: { isAdmin } });
    res.json({ status: "ok", message: "Admin status updated" });
  } catch (err) {
